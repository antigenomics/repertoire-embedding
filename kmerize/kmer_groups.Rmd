...
$\kappa_1,\kappa_2 \rightarrow d_{12}$. And storing (dense) $d_{12}$ matrix 
takes lots of space even for $k=3$.
...
cdr3_to_kmers <- function(cdr3, k = KMER_SZ) {
...
cdr3_to_kmers_gap <- function(cdr3, k = KMER_SZ) {
  pos <- 1:(nchar(cdr3) - k + 1)
  
  k_mid <- round(k / 2)
  kmer <- sapply(pos, function(x) paste0(substr(cdr3, x, x + k_mid - 1),
                                         "X", 
                                         substr(cdr3, x + k_mid + 1, x + k - 1),
                                         collapse = ""))
  data.table(cdr3, pos, kmer)
}
cdr3_to_kmers_gap("CASSLAPGATNEKLFF", 5)

cdr3s_to_kmers <- function(cdr3s, k = KMER_SZ, cores = CORES) {
  cdr3s[nchar(cdr3s) >= k] %>%
    as.list() %>%
